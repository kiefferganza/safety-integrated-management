...
// @mui
const { Card, Table, TableBody, Container, TableContainer } = await import(
    "@mui/material"
);
// routes
...
import {
    ReportTableRow,
    ReportTableToolbar,
} from "@/sections/@dashboard/ppe/list";
...
const TABLE_HEAD = [
    ...
    { id: "conducted_by", label: "Inventoried By", align: "left" },
...
const PPEReportListPage = ({
    ...
    const getReportType = (report) => {
        ...
        if (
            report.reviewer_id === user?.emp_id &&
            report.status === "for_review"
        ) {
            return "review";
        }
        if (
            report.approved_id === user?.emp_id &&
            report.status !== "for_review"
        ) {
            return "approve";
        }
...
function applyFilter({
    ...
    if (filterStartDate && !filterEndDate) {
        ...
        inputData = inputData.filter(
            (report) =>
                new Date(report.submitted_date).setHours(0, 0, 0, 0) >=
                startDateTimestamp
        );
    }
