...
const CompanyListPage = ({ companies }) => {
    ...
    useEffect(() => {
        if (companies && companies.length > 0) {
            const data = companies.map(
                ({ company_name, created_at, company_id }, index) => ({
                    id: company_id,
                    index: index + 1,
                    company_name,
                    created_at,
    ...
    const handleUpdateCompany = () => {
        Inertia.put(
            `/dashboard/company/${editCompanyData.id}/edit`,
            { company: companyName },
            {
                onStart: () => {
...
function applyFilter({ inputData, comparator, filterName, filterDate }) {
    ...
    if (filterName) {
        inputData = inputData.filter(({ company_name }) =>
            company_name.toLowerCase().includes(filterName.toLowerCase())
        );
    }
    ...
    if (filterDate) {
        ...
        inputData = inputData.filter(
            ({ created_at }) => fDate(created_at) === dateFiltered
        );
    }
