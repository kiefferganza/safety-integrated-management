...
const { ToolboxTalkTableRow, ToolboxTalkTableToolbar } = await import(
    "@/sections/@dashboard/toolboxtalks/list"
);
...
const ToolboxTalkListPage = ({
    ...
    const handleFilterType = (event) => {
        ...
        if (typeof value !== "string") {
            if (isAllChecked && !isAll) {
                ...
                isAll &&
                (newVal.length === 1 || newVal.at(-1) === "All")
            ) {
                newVal = [
                    "All",
                    "Civil",
    ...
    const handleDeleteRow = (id) => {
        ...
        Inertia.post(
            route("toolboxtalk.management.delete"),
            { ids: [id] },
            {
                onStart: () => {
    ...
    const handleDeleteRows = (selected) => {
        ...
        Inertia.post(
            ...
            { ids: selected },
            {
                onStart: () => {
                    load(
                        `Deleting ${selected.length} toolbox talks`,
                        "Please wait..."
                    );
                    queryClient.invalidateQueries({
                        queryKey: ["toolboxtalks", user.subscriber_id],
...
function applyFilter({
    ...
    if (filterName) {
        inputData = inputData.filter(
            (toolbox) =>
                toolbox.cms.toLowerCase().indexOf(filterName.toLowerCase()) !==
                    -1 ||
                toolbox.title
                    .toLowerCase()
    ...
    if (filterStartDate && !filterEndDate) {
        ...
        inputData = inputData.filter(
            (toolbox) =>
                new Date(toolbox.date_conducted).setHours(0, 0, 0, 0) >=
                startDateTimestamp
        );
    }
    ...
    if (filterType.length !== 0 && !filterType.includes("All")) {
        ...
        inputData = inputData.filter(
            (toolbox) => types.indexOf(+toolbox.tbt_type) !== -1
        );
    }
    ...
    if (filterStartDate && filterEndDate) {
        ...
        inputData = inputData.filter((toolbox) => {
            ...
            return (
                dateConducted >= startDateTimestamp &&
                dateConducted <= endDateTimestamp
            );
        });
...
function getAverage(data, start = null, end = null) {
    ...
    const tbtByMonth = data.reduce((acc, toolbox) => {
        if (
            fTimestamp(toolbox.date_conducted) >= startMonth &&
            fTimestamp(toolbox.date_conducted) <= endMonth
        ) {
            if (toolbox.date_conducted in acc) {
                ...
                acc[toolbox.date_conducted].hours +=
                    toolbox.participants.reduce((acc) => (acc += 9), 0);
                acc[toolbox.date_conducted].participants +=
                    toolbox.participants.length;
    ...
    const calculate = tbtByMonthArr.reduce(
        (acc, curr) => {
