...
import { getInhouseMatrix } from "@/utils/axios";
...
const index = ({ from, to, auth: { user } }) => {
    ...
    const { data } = useQuery({
        queryKey: ["in-house-matrix", { sub: user.subscriber_id, from, to }],
        queryFn: () => getInhouseMatrix({ from, to }),
    });
    ...
    return (
        <Suspense fallback={<LoadingScreen />}>
            <DashboardLayout>
                <Head>
                    <title>Training Internal Matrix</title>
                </Head>

                <Container maxWidth={themeStretch ? false : "lg"}>
                    <CustomBreadcrumbs
                        heading={"Internal Matrix"}
                        links={[
                            {
                                name: "Dashboard",
