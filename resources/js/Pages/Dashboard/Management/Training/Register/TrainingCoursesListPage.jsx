...
// sections
import { RegisterTableRow, RegisterTableToolbar } from '@/sections/@dashboard/training/list';
...
export default function TrainingCoursesListPage ({ courses }) {
	...
	const handleCreateCourse = ({ courseItem }) => {
		const courses = courseItem.map(course => ({ course_name: course.course_name }));

		Inertia.post(route('training.management.new_courses'), { courses }, {
			onStart: () => {
	...
	const handleUpdateCourse = ({ courseItem }) => {
		Inertia.post(route('training.management.update_course', courseItem[0].id), { course_name: courseItem[0].course_name }, {
			onStart: () => {
