...
export default function TrainingList({ trainings, module, url, type }) {
        ...
        if (trainings && trainings?.length > 0) {
            setTableData(
                trainings.map((training) => ({
                    ...
                    completed: checkTrainingComplete(
                        training.trainees,
    ...
    const getCertificateLength = (bool) =>
        dataFiltered.filter((data) => data.completed === bool).length;
    ...
    const TABS = [
        ...
        },
        {
            ...
            label: "Valid",
            color: "success",
            count: getLengthByStatus("Valid").length,
        },
        {
            value: "Soon to Expire",
            label: "Soon to Expire",
            color: "warning",
            count: getLengthByStatus("Soon to Expire").length,
        },
        {
            value: "Expired",
            label: "Expired",
            color: "error",
            count: getLengthByStatus("Expired").length,
        },
        {
            value: true,
            label: "Completed",
...
function applyFilter({
    ...
    if (filterStatus !== "all") {
        if (typeof filterStatus === "boolean") {
            inputData = inputData.filter(
                (training) => training.completed === filterStatus
            );
        } else {
            inputData = inputData.filter(
                (training) =>
                    filterStatus.trim().toLowerCase() ==
                    training.status.text.trim().toLowerCase()
            );
        }
    }
