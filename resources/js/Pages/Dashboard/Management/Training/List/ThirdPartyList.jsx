...
import { getThirdPartyTrainingStatus } from "@/utils/formatDates";
import {
...
export default function TrainingThirdPartyList({ trainings, url, type }) {
    ...
    const checkTrainingComplete = (trainees, files = []) => {
        return trainees.every(
            (tr) => files.findIndex((f) => f.emp_id === tr.employee_id) !== -1
        );
    };
        ...
        if (trainings && trainings?.length > 0) {
            setTableData(
                trainings.map((training) => {
                    const completed = checkTrainingComplete(
                        training.trainees,
                    ...
                    return {
                        ...
                        cms: training?.project_code
                            ...
                            : "N/A",
                        status: getThirdPartyTrainingStatus(
                            training.date_expired,
                            completed
                                ? training.external_status.status === "closed"
                                : completed
                        ),
                        completed,
                    };
    ...
    const getLengthByStatus = (status) =>
        dataFiltered.filter((item) => item.status.text === status);

    const getPercentByStatus = (status) =>
        (getLengthByStatus(status).length / dataFiltered.length) * 100;

    const getLengthByStatusNotFiltered = (status) =>
        tableData.filter((item) => item.status.text === status);

    const getTotalHours = (trainings_arr) =>
        trainings_arr.reduce((acc, curr) => (acc += curr.training_hrs), 0);

    const getCertificateLength = (bool) =>
        tableData.filter((data) => data.completed === bool).length;

    const relatedList = {
        requested_by: trainings.filter(
            (training) => training.external_details.requested_by === user.emp_id
        ).length,
        reviewed_by: trainings.filter(
            (training) => training.external_details.reviewed_by === user.emp_id
        ).length,
        approved_by: trainings.filter(
            (training) => training.external_details.approved_by === user.emp_id
        ).length,
    };
    ...
    const TABS = [
        ...
        },
        {
            ...
            label: "Pending",
            color: "warning",
            count: getLengthByStatusNotFiltered("Pending").length,
        },
        {
            value: "Valid",
            label: "Valid",
            color: "success",
            count: getLengthByStatusNotFiltered("Valid").length,
        },
        {
            value: "Soon to Expire",
            label: "Soon to Expire",
            color: "warning",
            count: getLengthByStatusNotFiltered("Soon to Expire").length,
        },
        {
            value: "Expired",
            label: "Expired",
            color: "error",
            count: getLengthByStatusNotFiltered("Expired").length,
        },
        {
            value: "Completed",
            label: "Completed",
            color: "success",
            count: getCertificateLength(true),
        },
        {
            value: "Incomplete",
            label: "Incomplete",
            color: "warning",
            count: getCertificateLength(false),
        },
    ];
    ...
    const handleDeleteRow = (id) => {
        Inertia.post(
            route("training.management.destroy"),
            { ids: [id] },
            {
                preserveState: true,
                preserveScroll: true,
                ...
                <Card sx={{ mb: 5 }}>
                    <Scrollbar>
                        ...
                        >
                            ...
                            <TrainingAnalitic
                                title="Pending"
                                total={getLengthByStatus("Pending").length}
                                hours={getTotalHours(
                                    getLengthByStatus("Pending")
                                )}
                            ...
                            <TrainingAnalitic
                                ...
                                total={
                                    getLengthByStatus("Soon to Expire").length
                                }
                                hours={getTotalHours(
                                    getLengthByStatus("Soon to Expire")
                                )}
...
function applyFilter({
    ...
    if (filterName) {
        inputData = inputData.filter(
            (training) =>
                training.cms.toLowerCase().indexOf(filterName.toLowerCase()) !==
                    -1 ||
                (training?.course
    ...
    if (filterStatus !== "all") {
        inputData = inputData.filter(
            (training) =>
                training.external_details[filterStatus] === user.emp_id
        );
    }

    if (filterStatusTab !== "all") {
        if (
            filterStatusTab === "Completed" ||
            filterStatusTab === "Incomplete"
        ) {
            inputData = inputData.filter(
                (training) =>
                    training.completed === (filterStatusTab === "Completed")
            );
        } else {
            inputData = inputData.filter(
                (training) => training.status.text === filterStatusTab
            );
        }
    }
