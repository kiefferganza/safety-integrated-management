...
export default function UserListPage({ users }) {
    ...
    const [tableData, setTableData] = useState(() =>
        users.map((user) => ({ ...user, name: getCurrentUserName(user) }))
    );
    ...
    const STATUS_OPTIONS = [
        ...
        },
        {
            ...
            label: "Deactivated",
            color: "error",
            count: getLengthByStatus("deactivated"),
        },
    ];
                ...
                onFinish() {
                    ...
                    if (page > 0) {
                        if (selected.length === dataInPage.length) {
                            ...
                            const newPage =
                                Math.ceil(
                                    (tableData.length - selected.length) /
                                        rowsPerPage
                                ) - 1;
                            setPage(newPage);
                        }
...
function applyFilter({
    ...
    if (filterCreatedDate !== null) {
        inputData = inputData.filter(
            (user) => fDate(user.date_created) === filterCreatedDate
        );
    }
