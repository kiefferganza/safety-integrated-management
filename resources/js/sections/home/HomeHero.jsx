...
import { Button, Box, Link as MuiLink, Container, Typography, Stack, Grid } from '@mui/material';
// routes
...
const StyledRoot = styled('div')(({ theme }) => ({
	...
	...bgGradient({
		...
		imgUrl: '/storage/assets/background/overlay_2.jpg',
	}),
				...
				<Stack spacing={2} direction="row" justifyContent="center">
					{['sketch', 'figma', 'js', 'ts', 'nextjs'].map((platform) => (
						<m.div key={platform} variants={varFade().in}>
							<SvgColor src={`/storage/assets/icons/platforms/ic_${platform}.svg`} />
						</m.div>
					))}
				</Stack>
			</Stack>
		...
		>
			<Stack component={m.div} variants={varFade().in} sx={{ width: 344, position: 'relative' }}>
				<Box
					...
					src={`/storage/assets/images/home/hero_${isLight ? 'light' : 'dark'}_1.png`}
			...
			<Stack component={m.div} variants={varFade().in} sx={{ width: 720, position: 'relative', ml: -2 }}>
				<Box
					...
					animate={{ y: ['100%', '0%'] }}
					transition={transition}
					src={`/storage/assets/images/home/hero_${isLight ? 'light' : 'dark'}_2.png`}
					sx={{ position: 'absolute' }}
				/>
				<Box
					component={m.img}
