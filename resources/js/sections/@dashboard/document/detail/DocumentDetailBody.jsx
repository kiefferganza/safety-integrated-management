...
const { DocumentApproveFailDialog } = await import(
    "../portal/DocumentApproveFailDialog"
);
const { DocumentCommentDialog } = await import(
    "../portal/DocumentCommentDialog"
);
...
const DocumentDetailBody = ({ document, docType, user, positions }) => {
    ...
    const [openUpdateSubmitterFileInfo, setOpenUpdateSubmitterFileInfo] =
        useState(false);
    const [selectedStatus, setSelectedStatus] = useState("");
    const [actionResponseId, setActionResponseId] = useState(null);

    const docStat = document.approval_sign
        ? getDocumentReviewStatus(document.status)
        : getDocumentStatus(document.status);
    // reviewer
    const canReviewStatus = checkCanReview({ docType, document, user });
    // approval
    const approvalPos = document.approval_employee
        ? positions.find(
              (pos) => pos.position_id === document.approval_employee.position
          ).position
        : null;
    ...
    const handleDeleteComment = (commentId) => {
        const commentLength = document.comments.filter(
            (com) => com.reviewer_id === user?.employee?.employee_id
        ).length;
        Inertia.post(
            PATH_DASHBOARD.fileManager.deleteComment(commentId),
            { commentLength },
            {
                preserveScroll: true,
                onStart() {
    ...
    const isAlreadySigned =
        document.comments.findIndex(
            (com) =>
                document.reviewer_employees.findIndex(
                    (rev) => rev.employee_id === com.reviewer_id
                ) !== -1
        ) !== -1;
    const reviewerStatus = document.reviewer_employees.find(
        (revEmp) => revEmp.employee_id === user?.employee?.employee_id
    )?.pivot?.review_status;
        ...
        <>
            <Stack>
                    ...
                    <Grid item xs={6}>
                        <Stack alignItems="center">
                            ...
                            <Typography
                                ...
                                sx={{ color: "text.disabled" }}
                            >
                                1 = action required on this issue, 2 = advisory
                                comment
                            </Typography>
                        </Stack>
                    </Grid>
                    <Grid item xs={6}>
                        <Stack alignItems="center">
                            ...
                            <Typography
                                ...
                                sx={{ color: "text.disabled" }}
                            >
                                i = Incorporated, ii = Evaluated and not
                                incorporated for reason stated
                            </Typography>
                        </Stack>
                    </Grid>
                </Grid>
                            ...
                            <TableBody>
                                {document.comments.length > 0 ? (
                                    document.comments.map((row, index) => {
                                        const reviewer =
                                            document.reviewer_employees?.find(
                                                (revEmp) =>
                                                    revEmp.employee_id ===
                                                    row.reviewer_id
                                            );
                        ...
                        <TableContainer sx={{ overflow: "unset" }}>
                            <Scrollbar>
                                <Table sx={{ minWidth: 960 }}>
                                    ...
                                    <TableBody>
                                        {document.external_comments.map(
                                            (row, index) => {
                                                const commentStatus =
                                                    row.status == 0
                                                        ? {
                ...
                <Typography variant="caption" sx={{ color: "text.disabled" }}>
                    Note: you can only choose status code when all of your
                    comments as a reviewer is closed.
                </Typography>
                <Box
                    display="grid"
                    gridTemplateColumns="repeat(3, 1fr)"
                    border={1}
                    sx={{ borderColor: "text.disabled", mt: 2 }}
                >
                    <ReviewStatus
                        onClick={() => {
                    ...
                    <ReviewStatus
                        ...
                        canReviewStatus={canReviewStatus}
                        isSelected={
                            (reviewerStatus || "").toLowerCase() === "e"
                        }
                    >
                        E. NOWC: No Objection with comments
                    </ReviewStatus>
                    <ReviewStatus
                        onClick={() => {
                            if (!canReviewStatus) return;
                        ...
                        canReviewStatus={canReviewStatus}
                        isSelected={
                            (reviewerStatus || "").toLowerCase() === "f"
                        }
                    >
                        F. Responded / Reviewed / Actioned
                    </ReviewStatus>
                </Box>
                <Divider sx={{ borderStyle: "dashed", my: 2 }} />
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={6}>
                        <TableContainer sx={{ overflow: "unset" }}>
                            <Table>
                                ...
                                <TableBody>
                                    {document?.reviewer_employees?.map(
                                        (revEmp, idx) => {
                                            const revPos = positions.find(
                                                (pos) =>
                                                    pos.position_id ===
                                                    revEmp.position
                                            )?.position;
                            ...
                            <Grid item xs={12}>
                                <TableContainer sx={{ overflow: "unset" }}>
                                    <Table>
                                        ...
                                        <TableBody>
                                            {document?.external_approver?.map(
                                                (app) => {
                                                    const appStatus =
                                                        getDocumentReviewStatus(
                                                            app.status
                                    ...
                                    <Button
                                        ...
                                        onClick={() =>
                                            handleOpenUpdateFile({
                                                name: "update-approval-file",
                                                id: document.approval_sign
                                                    .approval_sign_id,
                                            })
                                        }
                                    >
            ...
            <ReUploadFile
                ...
                routeName={route(
                    "files.management.document.reupload_submitter_file",
...
function checkCanReview({ docType, document, user }) {
    ...
    const currReviewSign = document?.reviewer_sign?.find(
        (revS) => revS?.user_id === user?.id
    );
    if (type === "review" && currReviewSign) return false;
    if (document.approval_sign !== null) return false;
    if (type !== "review") return false;
    const reviewerComments = document.comments.filter(
        (com) => com.reviewer_id === user?.employee?.employee_id
    );
