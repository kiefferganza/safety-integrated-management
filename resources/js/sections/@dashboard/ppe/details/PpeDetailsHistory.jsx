...
// mui
const { Table, TableBody, TableCell, TableContainer, TableRow } = await import(
    "@mui/material"
);
// utils
...
const { PpeDetailTableToolbar } = await import(
    "@/sections/@dashboard/ppe/details/PpeDetailTableToolbar"
);
    ...
    useEffect(() => {
        if (bound) {
            const data = bound.map((d) => ({
                ...d,
                // info: d.type === "inbound" ? ,
                requestedBy: d?.creator
                    ? d.creator?.fullname
                    : d?.requested_by_location,
            }));
            setTableData(data);
        }
    }, [bound]);
