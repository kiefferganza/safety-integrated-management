...
export default function PpePDF({ report, title = "PPE REPORT PREVIEW" }) {
    ...
    const overallTotal = report?.inventories
        ? report.inventories.reduce(
              (acc, curr) =>
                  acc +
                  (curr?.max_order || 0) * (curr?.item_price || curr?.price),
              0
          )
        : 0;

    const forcastMonth = report.budget_forcast_date
        ? `${fDate(
              startOfMonth(new Date(report.budget_forcast_date)),
              "dd"
          )} - ${fDate(
    ...
    return (
        <Document title={title}>
            {documents.map((doc, index) => {
                const total = doc.reduce(
                    (acc, curr) =>
                        acc +
                        (curr?.max_order || 0) *
                            (curr?.item_price || curr?.price),
                    0
                );
                    ...
                    <Page size="A4" style={styles.page} key={index}>
                        <View style={styles.mb16}>
                            ...
                            >
                                <Text style={styles.h4}>
                                    Monthly PPE Inventory Report & Budget
                                    Forecast Report
                                </Text>
                            </View>
                            <View style={styles.gridContainer}>
                        ...
                        <View style={styles.table}>
                            ...
                            <View style={styles.tableBody}>
                                {doc?.map((row, idx) => {
                                    const requestStatus = getInventoryStatus(
                                        (row?.max_order || 0) + (row?.qty || 0),
                                        row?.min_order || 0
                                    );
                                    return (
                                        <View style={styles.tableRow} key={idx}>
                                            ...
                                            <View style={styles.tableCell_2}>
                                                <Text style={{ fontSize: 6 }}>
                                                    {(
                                                        row?.outboundTotalQty ||
                                                        row?.outbound_total_qty ||
                                                        0
                                                    ).toLocaleString()}
                                                </Text>
                                            </View>
                                ...
                                {doc.length < PER_PAGE &&
                                    Array.from(
                                        Array(PER_PAGE - doc.length).keys()
                                    ).map((col, idx) => (
