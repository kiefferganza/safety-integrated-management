...
function generateDateRanges(startDate, endDate) {
    ...
    if (yearsArray.length > 1) {
        ...
        dateRange[
            yearsArray.at(0).getFullYear()
        ] = `${startMonth.toLocaleString("default", { month: "short" })} - Dec`;
        ...
        if (isLastDateJan) {
            dateRange[
                yearsArray.at(-1).getFullYear()
            ] = `Jan 1-${getDaysInMonth(endDate)}`;
            return dateRange;
        }
    ...
    if (startDate.getMonth() === endDate.getMonth()) {
        return {
            [year]: `${startMonth.toLocaleString("default", {
                month: "short",
            })} 1-${getDaysInMonth(startDate)}`,
        };
    }
...
export default function MatrixPDF({ years, titles, from, to }) {
            ...
            if (data?.length > PER_PAGE) {
                ...
                while (i < data.length) {
                    const isFirstOrLastPage =
                        i === 0 || i + PER_PAGE >= data.length;
                    const chunkSize = isFirstOrLastPage
                        ? FIRST_AND_LAST_PAGE
                        : PER_PAGE;
    ...
    for (const year in total) {
        ...
        if (yearTupple[1]) {
            ...
            total[year].completed.summary = populateSummary(
                total[year].completed,
                prevComp,
                coursesLowerObj
            );
            total[year].completed.summary.total =
                (total[year].completed.summary?.total || 0) +
                (total[year].completed.total +
                    (prevComp.summary?.total || prevComp.total));

            // not completed
            const prevNotComp = total[prevIndex].notCompleted;
            total[year].notCompleted.summary = populateSummary(
                total[year].notCompleted,
                prevNotComp,
                coursesLowerObj
            );
            total[year].notCompleted.summary.total =
                (total[year].notCompleted.summary?.total || 0) +
                (total[year].notCompleted.total +
                    (prevNotComp.summary?.total || prevNotComp.total));

            // not completed
            const prevExp = total[prevIndex].expired;
            total[year].expired.summary = populateSummary(
                total[year].expired,
                prevExp,
                coursesLowerObj
            );
            total[year].expired.summary.total =
                (total[year].expired.summary?.total || 0) +
                (total[year].expired.total +
                    (prevExp.summary?.total || prevExp.total));

            const prevHours = total[prevIndex].hours;
            total[year].hours.summary = populateSummary(
                total[year].hours,
                prevHours,
                coursesLowerObj
            );
            total[year].hours.summary.total =
                (total[year].hours.summary?.total || 0) +
                (total[year].hours.total +
                    (prevHours.summary?.total || prevHours.total));
        }
    }
            ...
            <View>
                <View style={styles.tableHead}>
                    ...
                    >
                        <View
                            style={[
                                styles.tableTextVerticialWrapper,
                        ...
                        >
                            ...
                            >
                                Total Course Completed per/ Personel
                            </Text>
                        </View>
                    </View>
                </View>
                {(data || [])?.map((d, i) => {
                    ...
                    return (
                        <View key={i} style={styles.tableBody}>
                            ...
                            {titles?.map((title, idx) => {
                                const course = d?.data?.find(
                                    (d) =>
                                        d?.courseName?.trim()?.toLowerCase() ===
                ...
                <View style={{ flexDirection: "row" }}>
                    ...
                    {titles?.map((title, idx) => (
                        ...
                        >
                            ...
                            >
                                {total[year]["hours"][title.toLowerCase()] || 0}
                            </Text>
                            {idx + 1 === titles.length && (
                                <Text
                                    style={[
                ...
                <View style={{ flexDirection: "row" }}>
                    ...
                    >
                        ...
                        >
                            Total Personel per/Course Completed
                        </Text>
                    </View>
                ...
                <View style={{ flexDirection: "row" }}>
                    ...
                    >
                        ...
                        >
                            Total Personel w/Inc. Requirements
                        </Text>
                    </View>
                    ...
                    {titles?.map((title, idx) => (
                        ...
                        >
                            ...
                            {idx + 1 === titles.length && (
                                ...
                                >
                                    {(
                                        total[year].notCompleted?.total || 0
                                    ).toLocaleString()}
                                </Text>
                            )}
                        </View>
                ...
                {total[year].notCompleted.summary &&
                    pageLength - 1 === pageIndex && (
                        <View
                            style={{ flexDirection: "row", marginVertical: 2 }}
                        >
