...
export default function ToolboxTalkNewEditForm({ tbt = {}, projectDetails }) {
    const isEdit = !!tbt?.tbt_id;
    ...
    const NewTBTSchema = Yup.object().shape({
        ...
        date_conducted: Yup.string()
            .nullable()
            .required("Conducted Date is required"),
        time_conducted: Yup.string()
            .nullable()
            .required("Time conducted is required"),
        ...
        participants: Yup.array()
            .of(
                Yup.object().shape({
                    ...
                    time: Yup.string().when("selected", (selected, schema) =>
                        selected
                            ? schema.nullable().required("Time is required")
                            : schema.notRequired()
                    ),
                })
            )
    ...
    const defaultValues = useMemo(
        () => ({
            sequence_no: tbt?.tbt_type
                ? sequences[tbt?.tbt_type]
                : sequences[tbt_type] || "",
            ...
            time_conducted: tbt?.time_conducted
                ? formatISO(
                      new Date(
                          1999,
                          1,
                          1,
                          tbt?.time_conducted.split(":")[0],
                          tbt?.time_conducted.split(":")[1],
                          0
                      ),
    ...
    function getParticipants() {
        ...
        const p = participants.map((p) => {
            let isSelected = null;
            if (tbt?.participants) {
                isSelected = tbt.participants.find(
                    (participant) => participant.employee_id === p.employee_id
                );
            }
            ...
            return {
                ...
                timeRaw: isSelected
                    ? date.setHours(
                          isSelected.pivot.time == 24
                              ? 0
                              : +isSelected.pivot.time
                      )
                    : null,
            };
        });
    ...
    const onSubmit = async (data) => {
        ...
        if (isEdit) {
            const removed_files = tbt.file.filter(
                (prevFile) =>
                    data.img_src.findIndex(
                        (f) => f.id == prevFile.tbt_img_id
                    ) === -1
            );
        ...
        Inertia.post(
            isEdit
                ? PATH_DASHBOARD.toolboxTalks.edit(tbt.tbt_id)
                : route("toolboxtalk.management.store"),
            newData,
            {
                preserveScroll: true,
                preserveState: true,
                ...
                onFinish() {
                    ...
                    queryClient.invalidateQueries({
                        queryKey: [
                            "toolboxtalks",
                            { type: +data.tbt_type, sub: user.subscriber_id },
                        ],
                    });
                },
            }
