...
const NewUserSchema = Yup.object().shape({
    ...
    password: Yup.string()
        ...
        .required("Password is must not be empty"),
...
export default function UserNewEditForm({ isEdit = false, user, employees }) {
    ...
    const handleAutocompleteName = (_, val) => {
        if (val) {
            ...
            const emp = employees.find(
                (emp) => emp.employee_id === val?.employee_id
            );
            ...
            setValue("about", emp?.about || "", {
                shouldDirty: true,
                shouldValidate: true,
            });
            setValue("emp_id", val?.employee_id, {
                shouldDirty: true,
                shouldValidate: true,
            });
            setValue("username", emp.firstname.toLowerCase() + random4digits, {
                shouldDirty: true,
                shouldValidate: true,
            });
        } else {
    ...
    const onSubmit = async (data) => {
        try {
            Inertia.post(
                isEdit
                    ? route("management.user.update", user.id)
                    : route("management.user.store"),
                data,
                {
                    onStart() {
                        setLoading(true);
                        ...
                        >
                            ...
                            {isEdit ? (
                                <RHFTextField
                                    name="password"
                                    label={
                                        isEdit ? "Update Password" : "Password"
                                    }
