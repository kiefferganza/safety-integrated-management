...
ProfileCover.propTypes = {
	...
	setOpenBackdrop: PropTypes.oneOfType([
		PropTypes.func,
		PropTypes.any,
	])
};

export default function ProfileCover ({ name, role, user, isCurrentProfile, setOpenBackdrop }) {
	...
	const handleDrop = useCallback(
		(acceptedFiles) => {
			...
			if (file) {
				Inertia.post(route('api.user.update_profile_image', user?.user_id), { profile_pic: file }, {
					preserveScroll: true,
					onStart () {
	...
	const handleDropCover = useCallback(
		(acceptedFiles) => {
			...
			if (file) {
				Inertia.post(route('api.user.add_cover', user?.user_id), { cover: file }, {
					preserveScroll: true,
					onStart () {
						if (typeof (setOpenBackdrop) === 'function') {
							setOpenBackdrop(true);
	...
	useEffect(() => {
		setProfileImage(() => user?.profile?.small || route("image", { path: "assets/images/default-profile.jpg", w: 128, h: 128, fit: "crop" }));
		setCoverImage(() => user?.cover?.cover || route("image", { path: "assets/images/home/cover.jpg", w: 1200, h: 280, fit: "crop" }));
	...
	return (
		<StyledRoot>
			...
			{isCurrentProfile ? (
				...
				<Image
					alt="cover"
					src={user?.cover?.cover || route("image", { path: "assets/images/home/cover.jpg", w: 1200, h: 280, fit: "crop" })}
					sx={{
						top: 0,
						left: 0,
						right: 0,
