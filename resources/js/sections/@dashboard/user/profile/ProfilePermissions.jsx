...
const ProfilePermissions = () => {
    ...
    const [expanded, setExpanded] = useState(() => {
        const titles = Object.keys(permissions).reduce(
    ...
    const handleTogglePermission = (event) => {
        ...
        Inertia.put(
            route("management.user.updateUserPermission", user.id),
            { name, isAllowed: checked },
            {
                preserveScroll: true,
                preserveState: true,
                onStart() {
    ...
    const handleUpdateUserStatus = () => {
        const newRoute =
            user.status === 1
                ? "management.user.deactivate"
                : "management.user.activate";
        Inertia.put(
            route(newRoute, user.id),
            {},
            {
                preserveScroll: true,
                preserveState: true,
                    ...
                    <Stack spacing={2}>
                        <ProfileAbout
                            ...
                            department={startCase(
                                user?.employee?.department?.department?.toLowerCase()
                            )}
                            name={(
                                                        ...
                                                        <TableCell>
                                                            ...
                                                            >
                                                                <Avatar
                                                                    ...
                                                                    src={
                                                                        row?.img_src
                                                                            ? `/storage/media/photos/employee/${row.img_src}`
                                                                            : null
                                                                    }
                                                                />
