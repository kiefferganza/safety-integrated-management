...
export default function StoreTableRow ({ row, selected, onSelectRow, onDeleteRow }) {
	const { auth: { user } } = usePage().props;
	const { user_id, name, thumbnail, created_at, updated_at, status, price, currency, qty, min_qty, unit, slug } = row;
	...
	if (currency in currencies) {
		currencySymbol = currencies[currency]?.symbol_native || currencies[currency]?.symbol || "IQD";
	}

	const canEdit = user.user_id === user_id;
	const canDelete = user.user_id === user_id;
	return (
		<>
			<TableRow hover selected={selected}>
				...
				<TableCell>
					<Stack direction="row" alignItems="center" spacing={2}>
						...
						<MuiLink component={Link} href={PATH_DASHBOARD.store.show(slug)} noWrap color="inherit" variant="subtitle2" sx={{ cursor: 'pointer', textTransform: 'capitalize' }}>
							{name}
						</MuiLink>
					</Stack>
				</TableCell>

				<TableCell align="left">{qty}/{min_qty}</TableCell>

				<TableCell align="left">{unit}</TableCell>

				<TableCell align="left">{currencySymbol} {fCurrencyNumber(price)}</TableCell>
