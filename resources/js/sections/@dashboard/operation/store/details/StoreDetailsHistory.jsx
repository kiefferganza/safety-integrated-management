...
const TABLE_HEAD = [
	...
	{ id: 'requestedBy', label: 'Requested By / Location', align: 'left' },
	{ id: 'created_at', label: 'Created', align: 'left' },
	{ id: 'qty', label: 'Quantity', align: 'right' },
	{ id: 'prev_qty', label: 'Previous Quantity', align: 'right' },
];


export default function StoreDetailsHistory ({ history, filterName, setFilterName }) {
	const {
		dense,
		page,
		order,
	...
	useEffect(() => {
		if (history) {
			const data = history.map((d) => ({
				...d,
				requestedBy: d?.creator ? d.creator?.fullname : d?.location
			}));
			setTableData(data);
		}
	}, [history]);
								...
								>
									...
									<TableCell align="center">
										<Label
											variant="soft"
											color={
												(row.type === 'add' && 'success') || (row.type === 'outbound' && 'warning') || 'success'
											}
										>
											{row.type === "add" ? "Restocked" : " Pulled Out"}
										</Label>
									</TableCell>
									<TableCell>{row?.requestedBy}</TableCell>
									<TableCell>{fDateTime(row.created_at)}</TableCell>
									<TableCell align="right">{`${row.type === "add" ? "+" : "-"} (${row.qty})`}</TableCell>
									<TableCell align="right">{row.prev_qty}</TableCell>
								</TableRow>
							)}
...
function applyFilter ({ inputData, comparator, filterName, filterType, filterStartDate, filterEndDate }) {
	...
	if (filterName) {
		inputData = inputData.filter((product) =>
			product?.creator ? product?.creator?.fullname?.toLowerCase().indexOf(filterName.toLowerCase()) !== -1 :
				product?.location?.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
		);
	}
	...
	if (filterStartDate && !filterEndDate) {
		...
		inputData = inputData.filter(product => new Date(product.created_at).setHours(0, 0, 0, 0) >= startDateTimestamp);
	}
	...
	if (filterStartDate && filterEndDate) {
		...
		inputData = inputData.filter(
			(product) =>
				new Date(product.created_at).setHours(0, 0, 0, 0) >= startDateTimestamp &&
				new Date(product.created_at).setHours(0, 0, 0, 0) <= endDateTimestamp
		);
	}
	return inputData;
}