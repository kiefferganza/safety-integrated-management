		...
		<Card sx={{ textAlign: 'center', height: 1 }} variant="outlined">
			...
			{latestUploadedFile && (
				<Box display="flex" alignItems="center" justifyContent="center" sx={{ my: 1 }}>
					...
					<MUILink
						component="a"
						href={latestUploadedFile?.fullSrc || `/storage/media/docs/${latestUploadedFile.src}`}
						sx={{
							color: "text.primary"
						}}
						target="_file"
					...
					>
						...
						>
							<Avatar variant="rounded" sx={{ bgcolor: 'background.neutral', width: 36, height: 36, borderRadius: "9px" }}>
								<Box component="img" src={fileThumb(fileFormat(latestUploadedFile?.fullSrc || latestUploadedFile.src))} sx={{ width: 24, height: 24 }} />
							</Avatar>
